...
def get_memory_service():
    ...
    if uri := os.environ.get("MEMORY_SERVICE_URI"):
        if uri.startswith("agentengine://"):
            engine_id = uri.replace("agentengine://", "")
            from google.adk.memory import VertexAiMemoryBankService

            return VertexAiMemoryBankService(
                project=os.environ.get("GOOGLE_CLOUD_PROJECT", "qwiklabs-gcp-03-a5fda0a88d46"),
                location=os.environ.get("GOOGLE_CLOUD_LOCATION") or "us-central1",
                agent_engine_id=engine_id,
            )
    if agent_engine_id := (
        os.environ.get("MEMORY_BANK_ID") or os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID")
    ):
        from google.adk.memory import VertexAiMemoryBankService
